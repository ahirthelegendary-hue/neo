...
class DesktopOverlay:
    ...
    def __init__(self):
        import logging
        self.logger = logging.getLogger("NEO.DesktopOverlay")
        self.logger.setLevel(logging.DEBUG)

        if not self.logger.handlers:
            handler = logging.StreamHandler()
            formatter = logging.Formatter(
                "[%(asctime)s] [%(levelname)s] [DesktopOverlay] %(message)s"
            )
            handler.setFormatter(formatter)
            self.logger.addHandler(handler)

    def start(self):   # 👈 YE MISSING THA
        try:
            self.logger.info("Desktop Overlay started")
        except Exception as e:
...
GlobalDesktopOverlay = DesktopOverlay()